...
\thesissetup{
    ...
    keywords      = {CEP, Kafka, ZooKeeper, Esper, analýza, distribuovaný, detekce vzorů, událost},
    TeXkeywords   = {CEP, Kafka, ZooKeeper, Esper, analýza, distribuovaný, detekce vzorů, událost},
    date			 =	2016/05/30
}
\thesislong{abstract}{
Cílem práce je navrhnout softwarové řešení na detekování vzorů událostí v distribuovaném prostředí. Práce staví na principu komplexního zpracování událostí (Complex Event Processing). Danou aplikaci bude možné nasadit na libovolný počet zařízení. Každé zařízení bude moct být producentem i konzumentem proudu událostí. Události mohou být dvou typů. Zároveň bude možné na libovolné zařízení nasadit pravidlo pro detekování vzorů v proudu událostí. Platnost pravidla může být časově omezená. Při úspěšné detekci vzoru aplikace spustí příslušnou akci, která závisí na typu událostí. A to podrobnou analýzu na podmnožině uzlů nebo vypsání informací na výstup. Výsledné řešení bude otestováno na čtyřech strojích.
}

\thesislong{thanks}{
Rád bych poděkoval RNDr. Filipu Nguyenovi za vedení mé práce, cenné rady a vstřícný přístup při konzultacích. Poděkování patří také mým rodičům, kteří mě během studia ze všech sil podporovali.
}
...
V případech, kdy chceme odhalit aktuálně hrozící problém, je nutné analyzovat data v reálném čase. Častým řešením je sběr dat z centrálních uzlů v síti a jejich analýza. Cílem této práce je navrhnout softwarové řešení, které bude možno nasadit přímo na koncová zařízení, na nichž může vzniknout nějaký problém. Software bude umožňovat posílání dat mezi jednotlivými zařízeními a jejich analýzu přímo na některém z těchto uzlů. Důležitou vlastností bude detekování definovaných vzorů v datových proudech, neboli detekce komplexních událostí.
...
Takový proud událostí skrývá množství dat, která je možné získat až při komplexní analýze. Komplexní analýza zohledňuje více událostí v řadě. To nazýváme \textit{komplexní zpracování dat (angl. complex event processing neboli CEP).}
...
\begin{itemize}
  \item Distribuované zpracování událostí jako zpracování událostí z více heterogenních zdrojů (distribuovaný systém).~[\ref{bib_2}] Výpočet může běžet i na jenom stroji a samotná analýza většinou nebývá paralelní. Takto pojem chápe i \textit{David Luckham} v knize \textit{The Power of Events} [\ref{bib_1}]. V knize to dobře demonstruje obrázek \textit{1.1}.
  \item Distribuované zpracování událostí jako výpočet rozdělený na více menších, méně náročných úloh za účelem rychlejšího zpracování s využitím paralelismu\footnote{Takto zpracovává data například Twitter [\ref{bib_distributed_computing}]. A to jejich distribucí pomocí nástroje \textit{Apache Storm}}. Dále v práci budeme pojem chápat právě tímto způsobem.
\end{itemize}
...
\begin{figure}[H]
	\centering
    \includegraphics[width=0.43\textwidth, height=0.4\textheight]{images/samsa-job-detail.png}
...
Druhou technologií pro distribuované zpracování dat, kterou přestavím, je \textit{Apache Storm}. Je to otevřený software určený ke zpracování proudu událostí v reálném čase. Byl vyvinut společností Twitter a nyní je dostupný pod licencí \textit{Apache Software Foundation}.
...
Cílem této práce je vytvoření software pro distribuované zpracování událostí, který bude schopen detekovat komplexní události na základě pravidel. Aplikace by měla být schopna zvládnout libovolný počet připojených uzlů. Síť propojených uzlů je možné reprezentovat grafem G(V, E), kde V jsou uzly grafu, neboli konkrétní počítače, na kterých probíhá zpracování. E jsou pak orientované hrany grafu, ty reprezentují informaci o tom který počítač posílá data kterému. Každá hrana je označena příznakem \textit{$L_i$}. Ten označuje typ události, která je po dané hraně posílána. Konkrétním příkladem tak může být událost \textit{$L_1$}, představující zprávu typu \textit{chyba}. Ta není tak častá, ale její závažnost je vyšší. A událost \textit{$L_2$}, představující zprávu typu \textit{upozornění}. Ta je naopak častá, ale méně závažná.
...
\begin{figure}[H]
    ...
    \caption{Srovnání přístupu zasílání dat do klaudu (vlevo) a vytvoření klaud přímo na koncových zařízeních (vpravo)}
...
read data: {type = L2} from queue;			
	...
	\label{fig:send_data_pseudo} 
\end{minipage}
\end{center}

\subsection*{Vyhodnotit data na základě pravidla}
Přijatá data jsou analyzována a průběžně se vyhodnocuje, zdali některá sekvence událostí odpovídá aktuálně nasazenému pravidlu. Pokud ano, je vytvořena nová, komplexní, událost, obsahující potřebná data pro adekvátní reakci. Adekvátní reakcí je myšleno vytvoření nové skupiny nebo vypsání události do logu.
...
\section{Technologie}
Technologie mi byly doporučeny vedoucím práce. Každá z nich by se dala nahradit, ale myslím si, že právě tato kombinace (\textit{Java + Maven, Kafka, Esper, ZooKeepr}) je ideální, protože jsou navzájem kompatibilní. Namátkou zmíním některé vazby, které usnadňují vývoj: \textit{Esper, Kafka i ZooKeeper} mají rozhraní pro Javu, proto je \textit{Maven} ideální. \textit{Kafka} používá \textit{ZooKeeper} pro distribuci nastavení, apod.
...
\label{sec:kafka}
Apache Kafka je systém pro zasílání zpráv. [\ref{bib_5}] Kafka klastr může být rozdělený mezi několik počítačů, každý nazýváme \textit{broker}. Základem aplikačního rámce Kafka je fronta zpráv. Ta je reprezentována tématem (angl. topic) respektive oddílem (angl. partition). Při vytváření tématu udáváme kromě jejího jména, také kolikrát se má replikovat mezi \textit{brokery} a počet oddílů. Oddíl je menší jednotka než téma. Každé téma může být replikováno mezi brokery.

\subsection*{Konzument a producent}

Do Kafka klastru data zapisují \textit{producenti} a na druhé straně z ní data čtou \textit{konzumenti} (angl. producer and consumer). Zapisování dat producentem je přímočaré. Producent rozhoduje, do kterého tématu, či oddílu se má zpráva zapsat. Zprávy jsou ukládány do fronty, tedy způsobem FIFO \footnote{FIFO -- první zapsán, první přečten (angl. first in first out)}. Čtení zpráv závisí na aktuálním stavu. Konzumenty je možné seskupovat do skupin a podle toho se pak rozlišuje způsob čtení zpráv na "queuing" a "publish-subscribe". V modelu "queue" je zpráva poslána vždy jednomu z konzumentů. Naopak v modelu "publish-subscribe" je každá zpráva poslána všem konzumentům. V Kafka dokumentaci se říká [\ref{bib_5}]:
\begin{itemize}
...
\begin{figure}[H]
	\centering
    \includegraphics[width=0.55\textwidth, height=0.35\textheight]{images/kafka.eps}
...
\begin{itemize}
  \item Datový pozorovatel -- změní stav, když jsou změněna data v uzlu. To znamená někdo zavolal operaci "zapsání dat". Změna stavu obsahuje nově nastavená data
...
\section{Shrnutí}
Kapitola obsahuje přehlednou tabulku, která konkrétně ukazuje, jaká technologie je použita při implementaci daného požadavku z analýzy. A návrh schématu. Schéma je znázorněno na čtyřech strojích.
...
\begin{figure}[H]
    ...
    \caption{Schéma aplikace, které znázorňuje, jaké technologie jsou nasazeny na každém stroji a jaké mají mezi sebou vazby.}
...
Vhodným řešením by bylo přidat ke každému tématu i oddíly, které by rozlišovaly druh zpráv. V případě této práce by v každém tématu byly dva oddíly. Jedna pro jemnozrné (\textit{LEVEL2}) a druhá pro hrubozrné události (\textit{LEVEL1}). Oddíly je možné dynamicky přidávat a tím mít možnost na jednom stroji analyzovat události množství různých typů.
